...
const MEP_MOUNT=window.location.pathname.startsWith('/mep/')?'/mep':'';
const nativeFetch=window.fetch.bind(window);
window.fetch=(input,options)=>nativeFetch(MEP_MOUNT&&typeof input==='string'&&input.startsWith('/api/')?`${MEP_MOUNT}${input}`:input,options);
...
function download(url){const anchor=document.createElement('a');anchor.href=MEP_MOUNT&&url.startsWith('/api/')?`${MEP_MOUNT}${url}`:url;anchor.click()}
